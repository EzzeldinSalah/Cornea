<script lang="ts">
	...
	let sessions = $state<CoachSession[]>([]);
	...
	async function fetchSessions() {
		...
		sessions = (await res.json()) as CoachSession[];
	...
	async function createSession() {
		...
		const newSession = (await res.json()) as CoachSession;
	...
	function handleSessionChange(e: Event) {
		...
		currentSessionId = parseInt(target.value);
	...
	async function sendMessage() {
		...
		try {
			...
			messages = [
				...messages,
				{ role: 'assistant', text: data.reply || 'API Error: No response.' }
			];
		} catch (e) {
			const message = e instanceof Error ? e.message : 'Unknown error';
			messages = [...messages, { role: 'assistant', text: `API Error: ${message}` }];
		} finally {
			loading = false;
			setTimeout(() => {
				if (chatContainer) chatContainer.scrollTop = chatContainer.scrollHeight;
...
<div class="page-header">
	<div class="header-content">
		...
		<div class="session-controls">
			<select
				class="session-select rounded-md"
