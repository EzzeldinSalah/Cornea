<script lang="ts">
	...
	function fmtLocal(vUsd: number) {
		return `${Math.abs(toLocal(vUsd)).toFixed(2)}`;
	}
	...
	{#if diff}
		<div class="stat-cards-grid mb-8">
			...
			<NeoCard class="stat-card">
				...
				<div class="stat-delta {diff.delta.received_usd >= 0 ? 'good' : 'bad'}">
					{sign(diff.delta.received_usd)}{fmtLocal(diff.delta.received_usd)}
					{$primaryCurrency}
			...
			<NeoCard class="stat-card">
				...
				<div class="stat-delta {diff.delta.effective_rate >= 0 ? 'good' : 'bad'}">
					...
					<span class="pct"
						>{fmtPct(pct(diff.base.effective_rate, diff.compare.effective_rate))}</span
					>
				</div>
			</NeoCard>
		...
		<div class="diff-table-container mb-8">
			<table class="git-diff-table">
				...
				<tbody>
					<tr
						class={diff.delta.invoiced_usd > 0
							? 'line-add'
							: diff.delta.invoiced_usd < 0
								? 'line-rm'
								: 'line-neutral'}
					>
						...
						<td
							>{sign(diff.delta.invoiced_usd)}{fmtUsd(diff.delta.invoiced_usd)}
							{fmtPct(pct(diff.base.invoiced_usd, diff.compare.invoiced_usd))}</td
						>
					</tr>
					<tr
						class={diff.delta.received_usd > 0
							? 'line-add'
							: diff.delta.received_usd < 0
								? 'line-rm'
								: 'line-neutral'}
					>
						...
						<td
							>{sign(diff.delta.received_usd)}{fmtUsd(diff.delta.received_usd)}
							{fmtPct(pct(diff.base.received_usd, diff.compare.received_usd))}</td
						>
					</tr>

					<tr
						class={diff.delta.fees_usd < 0
							? 'line-add'
							: diff.delta.fees_usd > 0
								? 'line-rm'
								: 'line-neutral'}
					>
					...
					<tr
						class={diff.delta.avg_payment_wait < 0
							? 'line-add'
							: diff.delta.avg_payment_wait > 0
								? 'line-rm'
								: 'line-neutral'}
					>
					...
					<tr
						class={diff.delta.client_count > 0
							? 'line-add'
							: diff.delta.client_count < 0
								? 'line-rm'
								: 'line-neutral'}
					>
		...
		{#if diff.base.clients.length > 0 || diff.compare.clients.length > 0}
			...
			<div class="client-diff-grid mb-8">
				<div class="client-col">
					<h3>Base Clients</h3>
					{#each diff.base.clients as client (client.name)}
				...
				<div class="client-col">
					...
					{#each diff.compare.clients as client (client.name)}
