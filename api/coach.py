...
def generate_coach_response(context_data: str, user_message: str) -> str:
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        return "You need to set the GEMINI_API_KEY in your .env file ya ebny."
    
    genai.configure(api_key=api_key)
    
    prompt = system_prompt.format(context=context_data)
